//===-- SimpleTempfileChecker.cpp ----------------------------------*- C++ -*--//
//
// The checker that is responsible for CWE-459.
//
// The implementation is based on SimpleFileDescriptorChecker.
//
// Defines a checker for proper use of mkstemp/unlink APIs.
//   - If a tempfile was generated with mkstemp, it must be deleted with unlink
//   before the execution ends. Failing to do so results in a resource leak.
//
//===----------------------------------------------------------------------===//
...
SimpleFileDescriptorChecker::SimpleFileDescriptorChecker()
    : OpenFn("mkstemp", 1), CloseFn("unlink", 1) {
...
void SimpleFileDescriptorChecker::checkPreCall(const CallEvent &Call,
  ...
  // Check if the tempfile has already been unlinked.
  ProgramStateRef State = C.getState();
  ...
  // Generate the next transition, in which the tempfile is unlinked.
...
void SimpleFileDescriptorChecker::reportDoubleClose(SymbolRef FileDescSym,
  ...
  auto R = std::make_unique<PathSensitiveBugReport>(
      *DoubleCloseBugType, "Deleting a previously deleted tempfile", ErrNode);
...
void SimpleFileDescriptorChecker::reportLeaks(ArrayRef<SymbolRef> LeakedFileDescriptors,
  ...
  for (SymbolRef LeakedFileDescriptor : LeakedFileDescriptors) {
    auto R = std::make_unique<PathSensitiveBugReport>(
        *LeakBugType, "Created tempfile is never deleted; potential resource leak",
        ErrNode);
...
void ento::registerSimpleTempfileChecker(CheckerManager &mgr) {
  mgr.registerChecker<SimpleFileDescriptorChecker>();
}

// This checker should be enabled regardless of how language options are set.
bool ento::shouldRegisterSimpleTempfileChecker(const CheckerManager &mgr) {
  return true;
}
